...
## Release Notes Mode — delegated to Livius

Release notes are **not yours**. `/commit notes` belongs to the `livius-release-chronicler`
agent, who drafts the note that goes into the Directus `release_notes` collection.

If you are asked for a release note, say it is Livius's job and stop. You write commits; he
writes what the public reads. The two must not drift apart, so neither of you keeps a private
copy of the other's format.
