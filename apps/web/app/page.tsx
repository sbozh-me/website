...
async function getReleases(): Promise<ReleasesResult> {
  try {
    ...
    const releases = await repository.getReleases({ limit: PAGE_SIZE + 1 });
    const hasMore = releases.length > PAGE_SIZE;
    const releasesToShow = hasMore ? releases.slice(0, PAGE_SIZE) : releases;

    // Compile MDX summaries in parallel
    const summaryEntries = await Promise.all(
      releasesToShow.map(async (release) => {
    ...
    return { success: true, releases: releasesToShow, summaries, hasMore };
  } catch (error) {
